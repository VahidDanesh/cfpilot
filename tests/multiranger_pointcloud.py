...
URI = uri_helper.uri_from_env(default='radio://0/88/2M/E7E7E7E7F0')
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, URI):
        ...
        # start with a bit higher default hover height
        ...
        # Flags to manage landing / stopped state
        self.landing = False
        self.stopped = False

    def sendHoverCommand(self):
        # Always try to send hover setpoint (restarts motors if needed)
        # If we've issued a stop (landing) request, don't send hover setpoints
        # so the craft stays landed until an explicit takeoff command.
        ...
        try:
            ...
            # If commander isn't available, ignore and continue
            pass
    ...
    def updateHover(self, k, v):
        ...
        if k == 'land':
            ...
            # mark stopped and landing state
            self.stopped = True
            self.landing = True
            return

        if k == 'esc' or k == 'escape':
            # immediate stop and close
            try:
                self.cf.commander.send_stop_setpoint()
                time.sleep(0.1)
                self.cf.commander.send_notify_setpoint_stop()
...
class Canvas(scene.SceneCanvas):
    ...
    def on_key_press(self, event):
        if (not event.native.isAutoRepeat()):
            ...
            if (event.native.key() == QtCore.Qt.Key_S):
                self.keyCB('height', -0.1)
            # Takeoff (T) and Land (L)
            if (event.native.key() == QtCore.Qt.Key_T):
                self.keyCB('takeoff', 1)
            if (event.native.key() == QtCore.Qt.Key_L):
                self.keyCB('land', 1)
            if (event.native.key() == QtCore.Qt.Key_Escape):
                self.keyCB('esc', 1)
    ...
    def on_key_release(self, event):
        if (not event.native.isAutoRepeat()):
            ...
            if (event.native.key() == QtCore.Qt.Key_X):
                self.keyCB('yaw', 0)
            # No continuous action needed on release, keep API consistent
            if (event.native.key() == QtCore.Qt.Key_T):
                self.keyCB('takeoff', 0)
            if (event.native.key() == QtCore.Qt.Key_L):
                self.keyCB('land', 0)
            if (event.native.key() == QtCore.Qt.Key_Escape):
                self.keyCB('esc', 0)
    ...
    def set_position(self, pos):
        ...
        if (PLOT_CF):
            ...
            # Request a redraw so the plot updates immediately
            try:
                self.update()
            except Exception:
                pass
    ...
    def set_measurement(self, measurements):
        ...
        # Ensure the canvas redraws when new measurements arrive
        try:
            self.update()
        except Exception:
            pass
